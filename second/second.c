...
void degreeOut(struct Node* graph[], long totalVertices, char vertex[]){
    long degree=0;
    for(long i=0; i<totalVertices; i++){
...
/*Function that finds & prints the in degree of the vertex*/
void degreeIn(struct Node* graph[], long totalVertices, char vertex[]){
    long degree=0;
    for(long i=0; i<totalVertices; i++){
        if(strcmp(graph[i]->data, vertex)!=0){
            struct Node* traverse = graph[i];
            while(traverse->next!=NULL){
                if(strcmp(traverse->next->data, vertex)==0){
                    degree++;
                }
                traverse=traverse->next;
            }
...
int main(int argc, char** argv){
    ...
    while(fscanf(fp2, "%c %s\n", &input, vertex)!=EOF){
        if(input=='o'){
            degreeOut(graph, totalVertices, vertex);
        }
        else if(input=='i'){
            degreeIn(graph, totalVertices, vertex);
        }
        else if (input=='a'){
